...
void gimbal_task(void const *pvParameters)
{
    ...
    //shoot init
    //射击初始化
    shoot_init(&shoot_current);
    //wait for all motor online
    //判断电机是否都上线
    while (0)//toe_is_error(YAW_GIMBAL_MOTOR_TOE) || toe_is_error(PITCH_GIMBAL_MOTOR_TOE))
    {
    ...
    while (1)
    {
        ...
        shoot_control_loop(&shoot_current);                
        
#if YAW_TURN//控制两个云台电机的旋转方向
        yaw_can_set_current = -gimbal_control.gimbal_yaw_motor.given_current;
#else
        yaw_can_set_current = gimbal_control.gimbal_yaw_motor.given_current;
#endif

#if PITCH_TURN//控制两个云台电机的旋转方向
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            { 
                CAN_cmd_gimbal(0, 0, 0, 0);
						...
						{	
						CAN_cmd_gimbal(yaw_can_set_current, pitch_can_set_current, shoot_current.trigger_give_current, 0);
						}
        }
        if(!(toe_is_error(FRIC_MOTOR1_TOE) && toe_is_error(FRIC_MOTOR2_TOE)))
        {
            if (toe_is_error(DBUS_TOE))
            { 
              ...
              CAN_cmd_fric(0,0,0,0);
						...
						else
						{
						  ...
						  CAN_cmd_fric(shoot_current.fric_give_current[0],shoot_current.fric_give_current[1],0,0);
...
/**
  * @brief          gimbal control mode :GIMBAL_MOTOR_ENCONDE, use the encode relative angle  to control. 
  * @param[out]     gimbal_motorcali_gimbal_hook: yaw motor or pitch motor
  * @retval         none
  */
/**
  * @brief          云台控制模式:GIMBAL_MOTOR_ENCONDE，使用编码相对角进行控制
