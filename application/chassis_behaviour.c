...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置模式
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//左拨动开关拨到中间
    {  
      chassis_behaviour_mode = CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW;
			//云台随动
			//chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
			// {
        // static uint16_t last_turn_keyboard_z = 0;
        // static uint8_t chassis_spin_flag = 0;
        // static fp32 chassis_end_angle = 0.0f;
        // //按shift键开小陀螺
        // if ((chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_SHIFT))
        // {
        //     if (chassis_spin_flag == 0)
        //     {
        //         chassis_spin_flag = 1;
        //     }
        // }
				// //按Z关
        // else if ((chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_Z))
        // {
        //     chassis_spin_flag = 0;
        // }
				
        // if (chassis_spin_flag)
        // {
				// 	chassis_behaviour_mode = 	CHASSIS_SMALL_SPIN ;
        // }
				
				// else if (chassis_spin_flag == 0){
				//   chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
				// }
			// }
    }
		
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//左拨动开关拨到底，不动
    {
			chassis_behaviour_mode = CHASSIS_NO_MOVE;
    }
		else if (chassis_move_mode->chassis_RC->key.v & SWING_KEY)
		{
        chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
		}
    else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))//左拨动开关拨到顶，小陀螺
    {
...
//		else if (chassis_behaviour_mode == 	CHASSIS_SMALL_SPIN )
//		{
//		chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW_SPIN;
//		}
...
//    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
//    *wz_set = -CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL]- chassis_move_rc_to_vector->chassis_RC->mouse.x * 0.1f;
//}
...
static void chassis_spin_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
	// 改写自chassis_no_follow_yaw_control
	if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_rc_to_vector == NULL)
	{
			return;
	}
	extern gimbal_control_t gimbal_control;
     gimbal_control.gimbal_yaw_motor.max_relative_angle =  5.0f;//max_yaw;
		 gimbal_control.gimbal_yaw_motor.min_relative_angle =  -5.0f;//min_yaw;
	chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
	ramp_calc(&chassis_move_rc_to_vector->chassis_spin_ramp, chassis_move_rc_to_vector->chassis_spin_ramp_add);
	//+ramp_calc(&chassis_move_rc_to_vector->chassis_spin_ramp, 0.5);
	*wz_set = chassis_move_rc_to_vector->chassis_spin_ramp.out;
	//*wz_set = 10;
}