...
void UI_SendByte(unsigned char ch)
{
	 //HAL_StatusTypeDef HAL_UART_Transmit(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, uint32_t Timeout);
		HAL_StatusTypeDef state=HAL_UART_Transmit(&huart6, &ch, sizeof(ch),100);//无符号char也是1byte=8bit
		if(state==HAL_OK){
				//提示发送成功
   ...
   //while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);	//发送完了，标志位 置1，此时判断“不等于1(SET)”==0(==RESET) 就可以
}
...
**********************************************************************************************************/
unsigned char *TESTframepoint;                      //读写指针
